...
export default function PelangganDashboard() {
  ...
  async function fetchDashboardData() {
    setLoading(true);
    try {
      // 1. Ambil Data Penitipan (DITAMBAHKAN !left agar data tidak hilang)
      const { data: penitipan, count: countPenitipan } = await supabase
        ...
        .select(`
          ...
          jenis_barang:jenis_barang_id!left (nama_jenis, harga_per_hari)
        `, { count: 'exact' })
      ...
      const combinedOrders = [
        ...
        const timeA = new Date(a.tanggal_referensi).getTime();
        const timeB = new Date(b.tanggal_referensi).getTime();
        return timeB - timeA;
      });

      // Update State Stats & Recent Orders
      setStats({
